...
import { addStream } from "../RTK/Slices/StreamSlice";
...
export default class SimpleSFUClient {
    ...
    async handleRemoteTrack(stream, username, consumerId) {
        ...
        if (userVideo) {
            const existingTracks = userVideo.srcObject.getTracks();
            const newTrack = stream.getVideoTracks()[0]; // Only consider video tracks
    
            if (!newTrack || existingTracks.some(track => track.id === newTrack.id)) {
                console.log('Video track already exists, skipping.');
                return; // Track already exists, so don't add it again
            }
            ...
            // Only add to store if it is not already present
            const existingStreams = store.getState().Stream.streams;
            if (!existingStreams.find(s => s.id === stream.id)) {
                store.dispatch(addStream([stream, username]));
            }
    ...
    async handleNewProducer({ id, username }) {
        ...
        // 🛠️ Check if this peer already exists
        if (this.clients.has(id)) {
            console.log(`Peer with id ${id} already exists, skipping consume.`);
            return; // Prevent adding duplicate consumers
        }
